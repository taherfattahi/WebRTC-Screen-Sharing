...
server.listen(httpPort, () => {
	console.log(`Https server listening at port ${httpPort}`);
});
...
sio.on('connection', (socket) => {
	...
	socket.on('disconnect', () => {
		registerUser = registerUser.filter(item => item.url.split("sessionid=")[1] != socket.sessionId);
		console.log(registerUser);
		sio.emit("getShareScreenStatus", registerUser);
	...
	socket.on("presenterStopSharing", (data) => {
		...
		registerUser = registerUser.filter(item => item.personalID !== data);
		console.log(registerUser);
		sio.emit("getShareScreenStatus", registerUser);
