export default class presenter {
    ...
    registerNodeEvents() {
        ...
        this.socket = io("https://vebko.org:3001", { query: 'session_id=' + sessionId });
    ...
    async startSharing(options, startEvent, stopEvent, failedEvent) {
        ...
        tracks.forEach(track => {
            if (track.kind == "video") {
                track.addEventListener('ended', () => {
                    ...
                    this.socket.emit("presenterStopSharing", this.personalID);
                    if (stopEvent) {
                        stopEvent.call();
                    }
                    this.stop();
        ...
        if (this.sharingOptions.mechanism == 'streamserver') {
			...
			url = url.replace("presenter.html", "viewer.html");
            let jsonData = {
                url,
                personalID: this.personalID,
            };
            this.socket.emit("presenterStartSharing", jsonData);
        }
