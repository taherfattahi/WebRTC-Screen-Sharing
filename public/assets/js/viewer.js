export default class viewer {
    ...
    registerNodeEvents() {
        ...
        this.socket = io("https://vebko.org:3001", { query: "mobile=" + this.mobileCheck() + '&session_id=' + sessionId });
    ...
    createPeerConnection(viewer) {
        ...
        pc.ontrack = async (event) => {
            if (viewer) {
				...
				setInterval(()=>{
					try{
						if(document.getElementById('plrVideo').paused){
							//document.getElementById('plrVideo').muted = false;
							document.getElementById('plrVideo').play();
						}
					}catch(err){}
				},2000);
